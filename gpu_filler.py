#!/usr/bin/env python3
"""Smart GPU filler for eval — keeps GPU utilization above cluster threshold.
...
scenario. Key differences from the training version:
  ...
  - vLLM coexistence: when vLLM is actively inferring, util is high → Tier 5
    auto-backoff kicks in; when vLLM is idle (barriers, prompt building, model
    switch), util drops → filler fills the gap
...
Four-tier fill strategy:
  ...
  Tier 2 — Low util, brief <0.5s:  gap matrix (2048) medium fill
  Tier 3 — Low util, sustained:    big matrix (4096) escalation
  Tier 4 — Below target util:      push matrix (3072/2048) dynamic fill

Usage:
    python gpu_filler.py                          # all GPUs, target 80%
    python gpu_filler.py --gpus 0,1,2,3           # specific GPUs
    python gpu_filler.py --target-util 85         # custom target
...
def monitor_loop(gpu_ids: list[int], nvml_map: dict[int, int], my_pid: int):
    """Poll NVML for all GPUs at ~2Hz (was 10Hz).
    ...
    """
    global _NVML_CONSECUTIVE_FAILURES
...
def filler_worker(
    ...
    Four-tier strategy (util-only, no process enumeration):
...
def main():
    parser = argparse.ArgumentParser(
        description="Smart GPU filler for eval — monitor-decoupled, 5-tier fill")
    ...
    print(f"[filler] mode: eval (no signal file, NVML-only detection)")
    print(f"[filler] architecture: monitor(2Hz+timeout) + workers(cache-read)")
